...
#RNA_samplesheet[,"sample"] <- gsub("_","-",RNA_samplesheet[,"sample"])
...
# dimRedu$week <- as.factor(dimRedu$week)
# levels(dimRedu$week) <- c("Week1","Week2","Week3","Week4","Week5","Week7","Abomasum","Lymph Node"  )

p=ggplot_dimRedu(dimRedu, sample_size="180", X, mtd)

#gplot_test=ggplot_dimRedu(dimRedu, sample_size="180", X, mtd)
#gplot_test
#ggplot_data_storage <- ggplot_build(gplot_test)$data
#levels(as.factor(ggplot_data_storage[[2]]$colour))
#ggsave(paste("~/Dropbox/BBSRC-ARC/figs/",X,mtd,"for Biopsy RNA-seq samples.pdf", sep=" "), width = 7, height = 6)
#ggsave(paste("~/Dropbox/wei_glasgow_work/vignette_BBSRC_ARC/paper_figs/",X,mtd,"for biopsy RNA-seq samples.png", sep=" "), width = 7, height = 6)

```


```{r, pca for biopsy, echo=F}
#dimRedu <- subset(dimRedu, dimRedu$week !="Week9"& dimRedu$week !="Lymph")
#remove the outliers
#dimRedu <- subset(dimRedu,dimRedu$name!="6615g1w1"&dimRedu$name!="6647g3w7"&dimRedu$name!="6576g3w7"&dimRedu$name!="6615g1w5")
# mds_cmd <- filter(mds_cmd, sample!="6595g4w3"&sample!="6923g4w2"&sample!="6576g3w3"&sample!="6652g3w3")
#ggplot_dimRedu(dimRedu, sample_size="biopsy (time series)", X, mtd)
```

```{r, error with rgl package}
  # rtsne_in <- t(as.matrix(unique(X_norm)))
  # rtsne_out_3 <- Rtsne(rtsne_in, dims=3)
  # dimRedu <- as.data.frame(rtsne_out_3$Y)
  # colnames(dimRedu) <- c("V1","V2", "V3")
  # dimRedu$group<- subset_samples$group
  # dimRedu$group[dimRedu$group=="Group1"] <- "3mo_vax"
  # dimRedu$group[dimRedu$group=="Group2"] <- "3mo_ctl"
  # dimRedu$group[dimRedu$group=="Group3"] <- "6mo_vax"
  # dimRedu$group[dimRedu$group=="Group4"] <- "6mo_ctl"
  # dimRedu$week <- subset_samples$week
  # dimRedu$name <- rownames(dimRedu)
  # scatterplot3d(x=dimRedu$V1, y=dimRedu$V2 ,z=dimRedu$V3,color=substring(dimRedu$week,5),angle = 180)
  #
  # plot3d(x=dimRedu$V1, y=dimRedu$V2 ,z=dimRedu$V3)
```
...
#pdf("~/Dropbox/BBSRC-ARC/figs/Total_worm_count.pdf", height=6, width=8)
# basic boxplot
#cols  <- colorRampPalette(brewer.pal(12, "Set3"), alpha=TRUE)(4)
#colsm <-matrix(rep(cols, each=4), ncol=4)
# boxplot(datTraits_worm$Total_Worms ~ datTraits_worm$Group, outcol="white", col=cols,
#         #col=c("rosybrown1","rosybrown4","paleturquoise1","paleturquoise4"),
#         main="Total Worm Count", xlab="Treatment", ylab="Worm Count",
#         names = c("3mo_Vax", "3mo_Ctrl","6mo_Vax", "6mo_Ctrl"))
# # Add data points
# mylevels<-levels(as.factor(datTraits_worm$Group))
# levelProportions<-summary(as.factor(datTraits_worm$Group))/nrow(datTraits_worm)
# for(i in 1:length(mylevels)){
#   thislevel<-mylevels[i]
#   thisvalues<-datTraits_worm[datTraits_worm$Group==thislevel, "Total_Worms"]
#   # take the x-axis indices and add a jitter, proportional to the N in each level
#   myjitter<-jitter(rep(i, length(thisvalues)), amount=levelProportions[i]/2)
#   points(myjitter, thisvalues, pch=19, col= t_col(cols[i])   ) #rgb(0,0,0,.2) )
# }
#dev.off()
...
#pdf("~/Dropbox/BBSRC-ARC/figs/Total_worm_count_protection_level.pdf", height=6, width=8)
...
#dev.off()
#write.csv(datTraits_worm_13, file="data/res/datTraits_worm_13.csv", col.names = T)
#write.csv(datTraits_worm, file="data/res/datTraits_worm.csv", col.names = T)
...
dev.off()
## Cumulated FEC in the last day
# basic boxplot
# pdf("~/Dropbox/BBSRC-ARC/figs/Final Faecal Egg Counts.pdf", width=8, height = 6)
# boxplot(datTraits_AUC$AUCDay95 ~ datTraits_AUC$Group, outcol="white",
#         col=c("rosybrown1","rosybrown4","paleturquoise1","paleturquoise4"),
#         main="Final FEC", xlab="Treatment", ylab="AUC",
#         names = c("3mo_Vax", "3mo_Ctrl","6mo_Vax", "6mo_Ctrl"))
# # Add data points
# mylevels<-levels(as.factor(datTraits_AUC$Group))
# levelProportions<-summary(as.factor(datTraits_AUC$Group))/nrow(datTraits_AUC)
# for(i in 1:length(mylevels)){
#   thislevel<-mylevels[i]
#   thisvalues<-datTraits_AUC[datTraits_AUC$Group==thislevel, "AUCDay95"]
#   myjitter<-jitter(rep(i, length(thisvalues)), amount=levelProportions[i]/2)
#   points(myjitter, thisvalues, pch=20, col=rgb(0,0,0,.2))
# }
# dev.off()
...
cols <- c("orangered3","lightpink2","royalblue3","skyblue1")
#cols <- c("orangered3","grey","royalblue3","grey")
...
<!-- ddsTC <- DESeqDataSetFromTximport(txi, sampleTable, design = ~condition) -->

<!-- resultsNames(ddsTC) -->
<!-- ## for control_y -->
<!-- resTC <- results(ddsTC, name="time_Week2_vs_Week1", test="Wald", alpha=0.05) -->
<!-- resTC<- results(ddsTC,  test="Wald", alpha = 0.1, -->
<!--                 contrast=list(c("time_Week2_vs_Week1","conditionVax.timeWeek5"))) -->
<!-- summary(resTC) -->
<!-- resTC <- results(ddsTC, contrast=list(c("condition_Vax_vs_Control","conditionVax.timeWeek3")), test="Wald",alpha = 0.05) -->
<!-- resTC <- results(ddsTC, name="conditionVax.timeWeek5", test="Wald", alpha = 0.1) -->
<!-- summary(resTC) -->
<!-- resTC <- results(ddsTC, name="condition_Vax_vs_Control", test="Wald",alpha = 0.05) -->
<!-- resTC <- results(ddsTC, name="Intercept", test="Wald",alpha = 0.05) -->

<!-- summary(resTC) -->
<!-- head(resTC[order(resTC$padj),],500) -->
...
for (ind_cl in 1:7) {
  ...
  # load(paste0("masigpro/get_",sel_group,".RData"))
  # gene_profile <- get$sig.genes$sig.profiles
  # gene_profile_sub <- gene_profile[match( gene_list[[ind_cl]][,1], rownames(gene_profile)),]
  # test <- PlotGroups(gene_profile_sub, edesign = get$sig.genes$edesign)
  # edesign = as.data.frame(get$sig.genes$edesign)
  # gene_mean <- vector(mode = "list", length=2)
  # gene_mean_df <- vector(mode = "list", length=2)

  # edesign_sub <- subset(edesign, Group1==1)
  # edesign_sub <- edesign_sub[order(edesign_sub$replicates),]
  # #gene_mean[[1]] <- matrix(0, 6, 8)
  # gene_mean[[1]] <- matrix(0, 6, 7)
  # #group1: i
  # #group 2: i+6
  # #group3: i+12
  # for (i in 1:6) {
  #   gene_profile_rep <- gene_profile_sub[,match(rownames(subset(edesign_sub, replicates==(i))), colnames(gene_profile_sub))]
  #   gene_mean[[1]][i,] <- log2(colMeans(gene_profile_rep))
  # }
  #
  # gene_mean[[1]] <- as.data.frame(gene_mean[[1]])
  # gene_mean_df[[1]] <- melt(t(gene_mean[[1]]))
  # colnames(gene_mean_df[[1]]) <- c("rep", "time", "gene")
  # gene_mean_df[[1]]$Group <- "Group1_3mo_vax"
  # #gene_mean_df[[1]]$Group <- "Group3_6mo_vax"
  # #gene_mean_df[[1]]$Group <- "Group4_6mo_control"
  # gene_mean_df[[1]]$mean <- 0
  # for (i in 1:6) {
  #   df_sub <- subset(gene_mean_df[[1]], time==i)
  #   gene_mean_df[[1]]$mean[gene_mean_df[[1]]$time==i] <- mean(df_sub$gene)
  # }
  #
  # edesign_sub <- subset(edesign, Group3==1)
  # #edesign_sub <- subset(edesign, Group2==1)
  # edesign_sub <- edesign_sub[order(edesign_sub$replicates),]
  # #gene_mean[[2]] <- matrix(0, 6, 7)
  # gene_mean[[2]] <- matrix(0, 6, 8)
  #
  # # group 3: i+12
  # # group 2: i+6
  # # group 4: i+18
  # for (i in 1:6) {
  #   gene_profile_rep <- gene_profile_sub[,match(rownames(subset(edesign_sub, replicates==(i+18))), colnames(gene_profile_sub))]
  #   gene_mean[[2]][i,] <- log2(colMeans(gene_profile_rep)) # log2
  # }
  # gene_mean[[2]] <- as.data.frame(gene_mean[[2]])
  # gene_mean_df[[2]] <- melt(t(gene_mean[[2]]))
  # colnames(gene_mean_df[[2]]) <- c("rep", "time", "gene")
  # gene_mean_df[[2]]$Group <- "Group4_6mo_ctrl"
  # #gene_mean_df[[2]]$Group <- "Group3_6mo_vax"
  # #gene_mean_df[[2]]$Group <- "Group2_3mo_ctrl"
  # gene_mean_df[[2]]$mean <- 0
  # for (i in 1:6) {
  #   df_sub <- subset(gene_mean_df[[2]], time==i)
  #   gene_mean_df[[2]]$mean[gene_mean_df[[2]]$time==i] <- mean(df_sub$gene)
  # }
  #
  # gene_df <- rbind(gene_mean_df[[1]], gene_mean_df[[2]])
  # gene_df$Group <- as.factor(gene_df$Group)
  # gene_df$time <- as.factor(gene_df$time)
  # plot_cl <- ggplot(gene_df, aes(x = time, y = gene))+
  #   geom_line(aes(x = time, y = mean, group=Group, color = Group), size=1.5)+
  #   geom_point(aes(color = Group, shape = Group), size=2) +
  #   theme(legend.position = "top")+
  #   scale_shape_manual(values=c(16,1))+
  #   scale_color_manual(values = c("lightpink1","skyblue1"))+
  #   #scale_color_manual(values = c("royalblue3","skyblue1"))+
  #   #scale_color_manual(values = c("orangered3","lightpink1"))+
  ...
  #   ylab("Averaged Log2 Gene Expression")+
  #   theme_bw()
  # ggsave(plot_cl, file=paste("masigpro/",sel_group,"_test/",ind_cl ,"_exp.png", sep=""), width=6, height=4)
  # dev.off()
}
...
# 1vs3
#kegg_result_sub <- subset(kegg_result, ID=="oas04060"|ID=="oas04664")

# 1vs2
#kegg_result_sub <- subset(kegg_result, ID=="oas04630"|ID=="oas04060"|ID=="oas04630"|ID=="oas04664"|ID=="oas04664"|ID=="oas04657")
...
#  geom_bar()+
#  coord_flip()+
#  scale_fill_manual(values=c("lightcoral", "lightskyblue", "lightgreen", "lightgoldenrod1"))+
#   #geom_text(aes(y=V2,label=V3), vjust=1.6,             color="white", size=3.5)+
# theme_bw()+theme(legend.text=element_text(size=14))+ggtitle("The distribution of DE genes in each cluster and treatments")+labs(y = "Number of Genes")
#   #ggsave("masigpro/stack plot of no of genes in each cluster and treatment.png", height = 4, width = 6)
#
...
#longData$group <- factor( longData$group, levels = colnames(ipa_table_comb)[-1])
...
#longData$log_p <- as.factor(logData$log_p)
#
...
p=ggplot(longData,aes(x = group, y = pathway, fill = log_p))+
 ...
 # coord_flip()+
  ...
  #ggsave("masigpro/ingenuity/pathway_heatmap/pathway_heatmap.png", height =20, width = 3)
